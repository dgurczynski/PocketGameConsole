...
public:
  ...
  void update() {
    ...
    float bot_speed = 1.8;
    if (p2_y + 8 < b_y) { p2_y += bot_speed; }
    else if (p2_y + 8 > b_y) { p2_y -= bot_speed; }
    ...
    if (b_y > 62 || b_y < 1) { b_vy *= -1.0; tone(buzzer, 2000, 10); }
    ...
    if (b_x > 127) { 
      p1_score++; 
      tone(buzzer, 250, 150);
      if(!is_game_over()) { reset_ball(-1); delay(1000); };
    }
    else if (b_x < 0) { 
      p2_score++;
      tone(buzzer, 250, 150);
  ...
  bool is_game_over() {
    return (p1_score >= 11 || p2_score >= 11);
  }
  ...
  void draw() {
    ...
    for (int i = 0; i < 64; i += 3) { display.drawPixel(63, i, WHITE); }
    ...
    if(is_game_over()) {
      ...
      if (p1_score == 11) { display.print("PLAYER 1 WON!"); }
      else if (p2_score == 11) { display.print("PLAYER 2 WON!"); }
    }
